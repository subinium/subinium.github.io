---
title : \[Keras Study\] 2장. 신경망 시작하기 (1)
category :
  - development
tag :
  - keras
...
책에서는 NVIDIA GPU를 권장한다.
GPU의 경우 다음과 같은 글을 참고하면 좋을 것 같다.
...
링크를 들어가면 딥러닝 공부 유형에 따라 GPU 추천해주고 있다. 작년까지만 해도 모두 GTX 1060을 가장 많이 추천해주었는데, 가격대가 안정되고 시간이 지남에 따라 선택할 수 있는 범위가 넓어진 것 같다.
...
RTX 2070은 가격대를 보니 60만원 내외인 것 같다. 쉽게 사기는 어려운 가격이나 고려는 해봐야겠다.
...
예전에 subinium.io 블로그를 만들때, 카드를 변경하며 요금을 못냈던 것이다. 계정이 막혀버리는 일이 발생했다!!! 계정을 버릴까 생각했지만, 어짜피 조만간 사이트를 하나 만들 생각이어서 지불할 생각이다.

기업이나 연구실에 들어가 필요한 기구를 막 사고 싶다. 장인은 장비를 탓하지 않지만, 난 장인이 아니니까!
공부 스폰서가 있으면 좋겠다는 생각!
아니면 돈을 벌어야하는데, 아직은 내가 어떤 것으로 돈을 벌 수 있을지 모르겠다.
