...
그리고 결과는 2가지를 MOS(귀로 듣고 score를 매김)로 측정합니다.
...
사실 이런 고민들과 이전의 방법론들을 follow-up하는데 1개월 정도 사용한 것 같습니다. 그리고 시작에 Taylor가 열심히 음성 기초에 대해 강의해주었는데 그게 엄청 큰 도움이 되었습니다. 이번에 카엔 블로그에 글도 쓰셨던데 많이 봐주시길 바랍니다.

- [AI에게 어떻게 음성을 가르칠까?](https://tech.kakaoenterprise.com/66)

그리고 최종적으로 많은 방법론 중 2가지 논문을 베이스로 baseline model을 만들기로 했습니다. (이유는 둘 다 결과가 마음에 들었기때문)
...
VQ-VAE를 베이스로 원래 뒤에는 Wavenet이 있어야 하는데, 뒤의 부분을 PWG로 변환했습니다. 제 실험 포인트는 이미 TTS용으로 어느 정도 검증된 Vocoder보다는 VQ 부분에서 Feature Extraction이 잘 되게 tuning하는 과정이 포인트였습니다.
