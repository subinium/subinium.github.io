---
title: "[Challenge] Voice Conversion Challenge 2020 후기"
category:
  - ML
tag:
  - Voice Conversion
...
![Model.001](https://i.imgur.com/ZBFxRDi.jpg)

## 나는 왜 음성처리파트로 배정되었는가?

카카오 블라인드 공채를 거쳐 카카오 브레인에서 추천을 받아 카카오 엔터프라이즈로 최종 면접 기회를 얻었고, 거기서 군필여부의 문제로 인턴으로 지원하여 합격한게 [이전까지의 이야기](/_posts/2019-10-08-kakao-blind-review.md)입니다.
...
![합격](https://i.imgur.com/9hiKxUN.jpg)

## 합격은 했는데...

![Model.002](https://i.imgur.com/r9Lzujz.jpg)

합격까지는 좋았지만 아무것도 모르는 인턴 Groot(나)는 첫 날에 상당히 부끄러웠습니다.

우선 음성합성과 Style Transfer를 하고 싶었던 만큼 Voice Conversion을 하게 되었습니다. 여기서 문제는 다음과 같습니다.

- Voice Conversion는 실서비스로 하기 어렵기에 회사의 main task가 되기 어렵다.
  - 어? 인턴인데 사수가 없다?!?
  - 어? 나 입사했는데 시키는게 없네?
- 그래서 내가 혼자 이 task를 캐리할 인재인가?
...
![없어요](https://i.imgur.com/blDYvBN.jpg)
...
모든게 처음이었지만, 저는 이런 어색함과 긴장을 즐기는 편입니다. (허세뿜뿜)

위에서 언급한 내용을 따라가기 위해 첫 3~4주간은 시작부터 끝까지 공부를 했습니다.
이미 딥러닝의 기본적인 이론은 알았기에 다음과 같은 내용으로 빠르게 공부했습니다.
...
그리고 결론적으로 첫 실험 후보군 논문은 2개의 논문으로 추려졌습니다.
...
"xx 모델을 돌려보았다"로 인턴을 끝내고 싶지는 않았고, 관련하여 주제를 조금 탐색하던 차에 [Voice Conversion Challenge 2020(이하 VCC2020)](http://www.vc-challenge.org/)가 열리는 것을 알았습니다.
...
하지만 대회 등록 기간이 2일 정도 남아있었고, 대회를 시작하고 얼마 후에 계약기간이 끝나는 시나리오 였습니다.

하지만 셀장님, 팀장님에게 인턴 연장을 부탁드렸고, 다행히 연장을 해주셔서 대회를 시작하게 되었습니다. 그리고 사수가 붙어 함께 대회를 진행하게 되었습니다.

### 대회소개 & 실험설계 : 시작은 했는데 막막하네

처음에는 잘 몰랐지만 Voice Conversion에서 follow-up 해야할 내용이 너무 많았습니다.

우선 대회는 2가지 task로 이뤄집니다.

- task 1 : 영어 화자 4명 -> 영어 화자 4명, 총 16 pairs
  - 여자와 남자에 대한 고려가 필요
- task 2 : 영어 화자 4명 -> 타 언어 화자 6명, 총 24 pairs
  - 이전 대회에는 없는 task
  - 언어는 핀란드어(Finnish), 독일어(German), 중국어(Mandarin)
  - 핀란드어/독일어/중국어 화자가 하는 영어로 변환

그리고 결과는 2가지를 MOS로 측정합니다.
...
- task 1과 task 2를 따로 진행할까?
  - 우선 화자별로 음성이 70개 밖에 없고, 대다수가 non-parallel한 데이터입니다.
  - 영어는 데이터셋도 어느 정도 있는 편이며 검증이 쉽다는 장점이 있음
  - 과연 task 1에 승산이 있을까?
    - VCC2018 우승 : 회사 내부 데이터쓴 중국 회사팀
...
- 여기서 말하는 Similarity란?
  ...
  - 이것에 대한 정답은?
- 언어들의 특징은?
  - duration이 Similairity에 큰 영향을 미칠까?
  - 핀란드어, 독일어는 뭐가 잘하는건지 모름
  - 중국어는 성조를 만들어야 함 (이건 ASR+TTS보다 잘하겠지 싶었습니다.)

사실 이런 고민들과 이전의 방법론들을 follow-up하는데 1개월 정도 사용한 것 같습니다.

그리고 최종적으로 많은 방법론 중 2가지 논문을 베이스로 baseline model을 만들기로 했습니다.

- Vocoder : [Parallel WaveGAN: A fast waveform generation model based on generative adversarial networks with multi-resolution spectrogram](https://arxiv.org/abs/1910.11480)
...
아마 baseline model에서 **_사람 목소리_** 가 나오고 나서는 거의 매일 `실험 -> 결과 확인 -> 개선`의 루틴을 살았습니다.

튜닝할 부분과 개선할 부분들을 찾으며 계속 실험했고, 대충 이런 과정으로 진행되었습니다.

![Model.015](https://i.imgur.com/wHUy7wx.jpg)

그림의 `/` 로 구분된 내용들은 모두 독립적으로 있으니 실험을 할 수 있는 가짓수는 기하급수적으로 늘어납니다. 매번 10~20개의 모델을 돌리고, tensorboard로 중간 결과를 듣고, 다시 돌리고...

그래도 GPU를 회사에서 많이 지원해주니 실험 자체에서 오는 답답함은 매우 적었습니다. 행복 그 자체...

그리고 실험 과정에서 Vocoder, 전처리, 데이터셋 등은 Taylor가 99% 해주셨기에 구현과 실험에 많이 집중할 수 있었습니다.

### 최종 모델

결과 제출에 모델에 대한 description 및 설명을 제출해야해서 열심히 만들어 제출했습니다.

그래서 나름 `PPT깎는개발자` 인 제가 열심히 만들어서 제출했습니다. (뿌듯한 그림이니 보고가세요.)
...
task2의 Similarity는 English, L2(not English) language를 Reference로 삼은 각각의 값입니다.
...
그래도 Similarity 결과를 보았을 때는 만족스러운 결과를 얻었습니다.
...
task 2인 cross-lingual voice conversion이 처음으로 VCC에 생긴 task인데 나쁘지 않은 성적을 거둔 것 같아 만족스럽습니다.

거기다 중국어는 1위라해도 무방한 결과라 더더욱 뿌듯합니다.
(성조를 고려하고, 혼자서 수백개의 음원을 청취평가한 보람이 있네요.)
...
대회에서 제공한 Team 결과에 Similarity(L2 Reference)에 모두 최상위권에 있다는 점이 만족 또 만족합니다.

이렇게 딥린이 인턴이 대회를 참여하는 스토리는 끝이 났습니다.

## 마무리

대회 결과가 출중히 좋다고 이야기할 수는 없으나 저는 후회 한 점 없이 뿌듯하고 행복한 경험으로 남았습니다. 실력도 늘었고, 딥러닝 자신감도 한껏 챙겼습니다.

![Model.018](https://i.imgur.com/QMFDuPj.jpg)

대회를 준비하며 이런 저런 도움들을 많이 받았습니다.
